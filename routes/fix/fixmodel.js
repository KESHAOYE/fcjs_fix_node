...
app.use('/GETMODELS', (req, res, next) => {
    ...
    let sql = model_name == '' ? `select f.model_id,f.model_name,f.model_img,f.brand_id,b.brandname,GROUP_CONCAT('{"sort_name":"',fs.sort_name,'"/"item_name":"',fi.item_name,'"/"price":"',m.price,'"}') as info from fixmodel f,model_item m,fixitem fi,fixitemsort fs,brandinfo b where f.delete = 0 and f.model_id = m.model_id and m.item_id = fi.item_id and fs.sort_id = fi.sort_id and f.brand_id = b.brandid limit ${start},${pageSize * page}` : `select f.model_id,f.model_name,f.model_img,f.brand_id,b.brandname GROUP_CONCAT('{sort_name:"',fs.sort_name,'",item_name:"',fi.item_name,'",price:"',m.price,'"}') as info from fixmodel f,model_item m,fixitem fi,fixitemsort fs,brandinfo b where f.delete = 0 and f.model_id = m.model_id and m.item_id = fi.item_id and fs.sort_id = fi.sort_id and f.brand_id = b.brandid and f.model_name like '%${model_name}%' limit ${start},${pageSize * page}`
...
app.use('/GETMODELBYID', (req, res, next) => {
    ...
    let sql = `select f.model_id,f.model_name,f.model_img,f.brand_id,b.brandname,GROUP_CONCAT('{"sort_name":"',fs.sort_name,'"/"item_name":"',fi.item_name,'"/"price":"',m.price,'"}') as info from fixmodel f,model_item m,fixitem fi,fixitemsort fs,brandinfo b where f.delete = 0 and f.model_id = m.model_id and m.item_id = fi.item_id and fs.sort_id = fi.sort_id and f.brand_id = b.brandid and f.model_id =  '${model_id}'`
    mysql(sql)
        .then(a => {
            a = JSON.parse(JSON.stringify(a))
            let data = a[0]
